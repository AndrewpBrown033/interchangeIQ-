...
export default function PlanModeView({
  ...
  return (
    <div className="space-y-6 relative font-sans select-none text-slate-900 w-full flex flex-col">
      
      {/* Top Page Header (Matching Game Day Top Action Header Style #030345) */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 p-4 rounded-2xl border border-blue-900/30 shadow-sm text-white" style={{ backgroundColor: '#030345' }}>
        {/* Left: Title & Context */}
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-black tracking-tight text-[#f2f3f6]">Visual Oval Plan</h2>
            ...
            <p className="text-xs text-blue-200 font-semibold mt-0.5">
              AFL oval pitch layout, bench interchanges, and quarter rotation planner • {teamName}
            </p>
          </div>
        </div>

        {/* Center: Plan Selector, Reference Clock & Quarter Switcher */}
        <div className="flex flex-wrap items-center gap-2.5 justify-between lg:justify-center">
          {/* Plan Selector */}
          <div className="flex items-center gap-1.5 bg-white/10 px-2.5 py-1 rounded-xl border border-white/15">
            <span className="text-xs font-bold text-blue-200">Plan:</span>
            <select
              value={selectedPlanId}
              onChange={(e) => setSelectedPlanId(e.target.value)}
              className="bg-slate-900 text-white font-extrabold text-xs px-2 py-0.5 rounded-lg border border-slate-700 focus:outline-none cursor-pointer"
            >
              {plans.map((p) => (
          ...
          {/* Quarter Clock */}
          <div
            className="flex items-center gap-1.5 bg-white/10 px-2.5 py-1 rounded-xl border border-white/15"
            title="Local reference clock for planning — independent from live Game Day match clock"
          >
            ...
            <button
              ...
              title={clockRunning ? 'Pause reference clock' : 'Start reference clock'}
            >
              {clockRunning ? <Pause className="w-3 h-3" /> : <Play className="w-3 h-3" />}
            </button>
            <button
              onClick={resetClock}
              className="p-1 rounded-md bg-white/10 hover:bg-white/20 text-slate-300 transition cursor-pointer"
              title="Reset clock"
            >
          ...
          <div className="flex bg-white/10 p-1 rounded-xl border border-white/15 gap-1">
            {[1, 2, 3, 4].map((q) => (
              <button
                ...
                className={`px-2.5 py-0.5 text-xs font-black rounded-lg transition cursor-pointer ${
                  selectedQuarter === q
        ...
        <div className="flex items-center gap-2 justify-end">
          <button
            ...
            className="px-3 py-1.5 text-xs font-black bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-xl transition cursor-pointer flex items-center gap-1.5 shadow-xs shrink-0"
          >
            <RefreshCw className="w-3.5 h-3.5" />
            <span>3-Way Builder</span>
          </button>

          {onNavigateToGameDay && (
            <button
              onClick={onNavigateToGameDay}
              className="px-3 py-1.5 text-xs font-black bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl transition cursor-pointer flex items-center gap-1.5 shadow-xs shrink-0"
          ...
          <button
            onClick={onClose}
            className="px-3 py-1.5 text-xs font-bold bg-white/10 hover:bg-white/20 text-white rounded-xl transition cursor-pointer flex items-center gap-1.5 shrink-0 border border-white/20"
      ...
      {/* Main Container Panel (Matching Game Day 3-Column Pitch Workspace Frame) */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden min-h-[620px] flex flex-col">
        ...
        <div className="flex md:hidden bg-slate-900 text-white border-b border-slate-800 p-1.5 gap-1 shrink-0">
          ...
          >
            <span>Queue ({qRotations.length})</span>
          </button>
        </div>
